exports.handler = async function(event) {
      ...
      if (hasEmail && hasPhone && hasConsent) {
        ...
        if (extractRes.ok) {
          ...
          if (et2 !== 'null' && et2.includes('{')) {
            try { const jm = et2.match(/\{[\s\S]*\}/); if (jm) { const parsed = JSON.parse(jm[0]); if (parsed.name && parsed.email && parsed.phone) { await syncToHubSpot(parsed, process.env.HUBSPOT_TOKEN); captured = true; } } } catch(e) { console.error('Extract error:', e.message); }
          }
        }
      }
    }
  ...
  } catch (err) { return { statusCode: 500, headers: { 'Access-Control-Allow-Origin': '*' }, body: JSON.stringify({ error: err.message }) }; }
};
